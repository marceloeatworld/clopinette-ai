...
- **Skills Hub** — install skills from the built-in catalog (10 curated skills) or trusted GitHub repos like Hermes, Cloudflare, MiniMax, and gstack. Installed skills are refreshed daily when their catalog or GitHub source changes
...
- Built-in catalog: `web-research`, `daily-briefing`, `document-qa`, `meeting-notes`, `code-review`, `git-workflow`, `api-design`, `debug-strategy`, `research-summary`, `writing-editor`. It ships inside the worker, so a deploy updates the hub for every user.
...
- Daily auto-update: each agent re-checks its catalog and GitHub installs by content hash and re-installs changed ones (URL installs are pinned at install time).
...
| `/digest on [Nh] \| off \| now \| to <number> \| status` | WhatsApp (Evolution) group digest, owner only: silently collect group messages and post a per-person summary on a schedule |
...
| Setting | Value |
|---|---|
| Workers AI tier | Kimi K3 (default) + Kimi K2.6 + Gemma 4 26B + GLM 4.7 Flash (free for trial / pro) |
| Internal / auxiliary | Gemma 4 26B — greeting fast path, compression, memory flush, self-learning, web summarization, browser snapshots |
...
# Codemode outbound proxy (only if you keep LOADER + CODEMODE_OUTBOUND in wrangler.jsonc)
(cd workers/outbound && wrangler deploy)
...
# Gateway integration (SaaS quota cache refresh; see clopinette-platform)
wrangler secret put GATEWAY_URL           # https://api.example.com
wrangler secret put GATEWAY_INTERNAL_KEY  # optional, falls back to WS_SIGNING_SECRET

# Discord behaviour (plain vars, not secrets)
# DISCORD_REQUIRE_MENTION=false          answer every guild message (default: mention only)
# DISCORD_FREE_RESPONSE_CHANNELS=id,id   channels that never require a mention
```

Slack: `SLACK_SIGNING_SECRET` enables signature verification on `/webhook/slack`, but the adapter is a stub (URL verification only).
...
src/
  ...
  prompt-builder.ts    Block-based system prompt (cached per session + platform)
