...
/**
 * Check hub-installed skills for updates.
 * - "github": compares stored content_hash with the current GitHub content.
 * - "catalog": compares with the inline catalog shipped in this deploy, so a
 *   catalog edit reaches every user's installed copy without a manual reinstall.
 * URL installs are not refreshed (arbitrary origins are pinned at install time).
 */
export async function checkSkillUpdates(
  ...
  const installed = sql<{ name: string; source: string; identifier: string; content_hash: string | null }>`
    SELECT name, source, identifier, content_hash FROM hub_installed WHERE source IN ('github', 'catalog')
  `;
  if (installed.length === 0) return result;

  const { GitHubSource } = await import("./github-source.js");
  const { getCatalogSkill } = await import("./catalog.js");
  const gh = new GitHubSource();

  for (const skill of installed) {
    result.checked++;
    try {
      let bundle: HubSkillBundle | null = null;
      if (skill.source === "catalog") {
        const entry = getCatalogSkill(skill.identifier);
        if (entry) bundle = { meta: entry.meta, content: entry.content, frontmatter: {} };
      } else {
        bundle = await gh.fetch(skill.identifier);
      }
      if (!bundle) continue;
