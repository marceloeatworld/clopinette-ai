...
export interface EvolutionContext {
  ...
  ingestGroupMessage: (chatJid: string, author: string, content: string) => void;
  /** Handle the owner-only /digest command; returns the reply text. */
  handleDigestCommand: (text: string, chatJid: string) => Promise<string>;
}
...
export async function handleEvolutionUpdate(
  ...
  // /digest is owner-only: the instance owner's messages (fromMe, i.e. typed on
  // the phone paired with this instance) are the only ones accepted, so group
  // members cannot control the digest.
  if (text.trim().startsWith("/digest")) {
    if (!fromMe) return new Response("ok");
    const reply = await ctx.handleDigestCommand(text.trim(), remoteJid);
    await sendEvolutionMessage(ctx.apiUrl, ctx.apiKey, ctx.instanceName, remoteJid, reply);
    return new Response("ok");
  }

  // All other owner messages are ignored (the bot acts as the owner's account)
  if (fromMe) return new Response("ok");

  // Digest mode: silently collect group messages — no pipeline, no reply.
  // Media is not downloaded or analyzed; only text and placeholders are kept.
  if (isGroup && ctx.isDigestChat(remoteJid)) {
    const author = payload.data.pushName
      ?? (payload.data.key.participant ? jidToPhone(payload.data.key.participant) : "unknown");
    let content = text;
    if (messageType === "imageMessage") content = content ? `[image] ${content}` : "[image]";
    else if (messageType === "audioMessage") content = "[voice message]";
    else if (messageType === "documentMessage") {
      const name = msg.documentMessage?.fileName ?? "file";
      content = content ? `[document: ${name}] ${content}` : `[document: ${name}]`;
    } else if (messageType === "locationMessage") content = "[location]";
    if (content) ctx.ingestGroupMessage(remoteJid, author, content.slice(0, 2000));
    return new Response("ok");
  }

  // Groups without digest mode: stay silent unless the bot is addressed by
  // name or with a command — a dedicated bot number added to a group must
  // not answer every message.
  if (isGroup && !text.startsWith("/") && !text.toLowerCase().includes("clopinette")) {
    return new Response("ok");
  }
