...
app.post("/webhook/telegram", async (c) => {
  ...
  // Use cached bot secret or fetch once. On a mismatch, re-read KV before
  // rejecting: /api/admin/setup-telegram rotates the secret, and a warm isolate
  // would otherwise keep 401-ing every webhook until it is recycled.
  if (!cachedBotSecret) {
...
app.post("/webhook/discord-bridge", async (c) => {
  ...
  // Reject stale requests (>5 min) to prevent replay attacks. A non-numeric
  // timestamp yields NaN, and `NaN > 300` is false — check finiteness explicitly.
  const age = Math.abs(Date.now() / 1000 - Number(timestamp));
  if (!timestamp || !signature || !Number.isFinite(age) || age > 300) {
    return new Response("Unauthorized", { status: 401 });
  }
...
app.post("/webhook/evolution", async (c) => {
  ...
  const rawBody = await c.req.text();
  let payload: { event?: string; instance?: string; apikey?: string; data?: { key?: { fromMe?: boolean; remoteJid?: string }; message?: { conversation?: string; extendedTextMessage?: { text?: string } } } };
  try { payload = JSON.parse(rawBody); } catch { return new Response("ok"); }
  ...
  // Only process incoming messages (not status updates). The owner's own
  // messages (fromMe) are ignored, except /digest which is owner-only.
  if (payload.event !== "messages.upsert") return new Response("ok");
  if (payload.data?.key?.fromMe) {
    const ownText = payload.data?.message?.conversation
      ?? payload.data?.message?.extendedTextMessage?.text ?? "";
    if (!ownText.trim().startsWith("/digest")) return new Response("ok");
  }
  ...
  if (!quota.allowed) {
    ...
    const msg = quota.reason === "payment_failed"
      ...
      : quota.reason === "monthly_limit" || quota.reason === "daily_limit"
        ? "Monthly usage limit reached. Check usage at clopinette.app/dashboard"
        : "WhatsApp bot is available on Pro and BYOK plans. Upgrade at clopinette.app/pricing";
    // Never post billing messages into group chats (one per member message)
    if (to && !to.endsWith("@g.us")) await sendEvolutionMessage(apiUrl, apiKey, instanceName, to, msg);
    return new Response("ok");
  }
