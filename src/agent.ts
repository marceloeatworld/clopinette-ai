...
export class ClopinetteAgent extends AIChatAgent<Env, AgentState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    // Same pattern as Telegram: acknowledge immediately and process in the DO
    // background. Meta retries webhooks that do not get a 200 within seconds, and
    // a long tool turn (web search, delegation) would otherwise be re-delivered
    // and answered twice.
    if (request.method === "POST" && url.pathname.includes("/webhook/whatsapp")) {
      ...
      this.ctx.waitUntil(handleWhatsAppUpdate(rawBody, {
        sql: this.sql.bind(this),
        env: this.env,
        sessionId: this.#sessionId!,
        userId: this.#userId,
    ...
    // Evolution API webhooks — self-hosted WhatsApp via Baileys (background, see above)
    if (request.method === "POST" && url.pathname.includes("/webhook/evolution")) {
      ...
      this.ctx.waitUntil(handleEvolutionUpdate(rawBody, {
        sql: this.sql.bind(this),
        env: this.env,
        sessionId: this.#sessionId!,
        userId: this.#userId,
        ...
        handleDigestCommand: (text, chatJid) => this.#handleDigestCommand(text, chatJid),
      }).catch(err => console.error("[evolution] handler error:", err)));
      return new Response("ok");
    }
  ...
  async wipeAccount(): Promise<{ ok: true; wiped: true }> {
    ...
    try { this.sql`DELETE FROM group_messages`; } catch { /* */ }
  ...
  async executeSkillUpdateCheck(): Promise<void> {
    try {
      ...
      if (result.updated > 0) {
        ...
        // Installed skill bodies changed — the skills index in the system prompt must follow
        this.#cachedSystemPrompts.clear();
      }
    ...
    this.sql`INSERT OR REPLACE INTO agent_config (key, value, encrypted, updated_at)
      VALUES ('_skill_update_checked_at', ${new Date().toISOString()}, 0, datetime('now'))`;
    // Re-schedule for next day
    await this.#scheduleSkillUpdateCheck();
  }

  /** Daily check. Runs shortly after start when the last check is older than a
   *  day (or never ran), so a catalog change in a deploy reaches every user on
   *  their next activity instead of up to 24h after their DO restarts. */
  async #scheduleSkillUpdateCheck(): Promise<void> {
    try {
      const DAY_MS = 24 * 60 * 60 * 1000;
      const rows = this.sql<{ value: string }>`SELECT value FROM agent_config WHERE key = '_skill_update_checked_at'`;
      const lastChecked = rows[0]?.value ? Date.parse(rows[0].value) : 0;
      const overdue = !Number.isFinite(lastChecked) || Date.now() - lastChecked >= DAY_MS;
      const nextRun = new Date(Date.now() + (overdue ? 30_000 : DAY_MS));
  ...
  async executeCronJob(payload: { jobId: string }): Promise<void> {
    ...
    // Group digest jobs have their own runner (transcript injection + note update)
    if (job.id.startsWith("digest:")) {
      try {
        await this.#runGroupDigest(job.id);
      } catch (err) {
  ...
  // ───────────────────────── Group Digest (WhatsApp via Evolution) ─────────────────────────

  #isDigestChat(chatJid: string): boolean {
    const rows = this.sql<{ id: string }>`
      SELECT id FROM cron_jobs WHERE id = ${`digest:${chatJid}`} AND enabled = 1`;
    return rows.length > 0;
  }

  /** Owner-only /digest command: on [Nh] | off | now | to <number> | status. */
  async #handleDigestCommand(text: string, chatJid: string): Promise<string> {
    if (!chatJid.endsWith("@g.us")) {
      return "The /digest command only works in group chats.";
    }
    const parts = text.split(/\s+/);
    const action = parts[1]?.toLowerCase() ?? "status";
    const jobId = `digest:${chatJid}`;

    if (action === "on") {
      let schedule = "0 18 * * *";
      let label = "daily at 18:00 UTC";
      const m = parts[2]?.toLowerCase().match(/^(\d{1,2})h$/);
      if (m) {
    ...
    if (action === "to") {
      if (!this.#isDigestChat(chatJid)) return "Digest is off for this group. Use /digest on first.";
      const number = (parts[2] ?? "").replace(/[+\s().-]/g, "");
      if (!/^\d{8,15}$/.test(number)) {
        return "Invalid phone number. Usage: /digest to 33612345678 (international format, digits only).";
      }
      this.sql`UPDATE cron_jobs SET deliver = ${`evolution:${number}`} WHERE id = ${jobId}`;
      return `Digest delivery set: summaries will be sent privately to ${number}.`;
    }

    if (action === "now") {
      if (!this.#isDigestChat(chatJid)) return "Digest is off for this group. Use /digest on to enable it.";
      const sent = await this.#runGroupDigest(jobId);
      return sent ? "Digest sent." : "No new messages since the last digest.";
    }

    // status
    const job = this.sql<{ schedule: string; deliver: string | null; last_run: string | null }>`
      SELECT schedule, deliver, last_run FROM cron_jobs WHERE id = ${jobId} AND enabled = 1`;
    if (job.length === 0) {
      return "Digest is off for this group. Use /digest on (daily at 18:00 UTC) or /digest on 6h.";
    }
    const pending = this.sql<{ cnt: number }>`
      SELECT COUNT(*) as cnt FROM group_messages WHERE chat_jid = ${chatJid}`;
    const target = job[0].deliver === `evolution:${chatJid}` ? "this group" : `private (${job[0].deliver?.split(":")[1] ?? "?"})`;
   ...
   * durable state). Returns true if a digest was sent, false if nothing new.
   */
  async #runGroupDigest(jobId: string): Promise<boolean> {
    const rows = this.sql<{ id: string; chat_id: string | null; platform: string | null; deliver: string | null }>`
      SELECT id, chat_id, platform, deliver FROM cron_jobs WHERE id = ${jobId} AND enabled = 1`;
    if (rows.length === 0) return false;
    const job = rows[0];
    const chatJid = job.chat_id ?? "";

    const messages = this.sql<{ id: number; author: string; content: string; created_at: string }>`
      SELECT id, author, content, created_at FROM group_messages
      WHERE chat_jid = ${chatJid} ORDER BY id ASC`;
    ...
    const prompt = [
      ...
      ``,
      `Tasks:`,
      `1. Extract the important items: direct requests, questions that need an answer, deadlines, appointments, decisions, and action items. Ignore small talk. Every item MUST be attributed to the person who said it (the name before the colon on each transcript line) — never report a request or question without naming its author.`,
      `2. Update the running note for this group with the note tool: search notes for "[Group digest] ${chatJid}". If it exists, edit it so it stays a current list of open items (drop resolved ones). Otherwise save a new note whose content starts with "[Group digest] ${chatJid}". Keep the author's name on every item in the note too.`,
      `3. Reply with the digest itself, grouped by person: one line per item, prefixed with the author's name, e.g. "- Marie: needs the invoice before Friday". If several people asked the same thing, name all of them. Write the digest in the same language as the messages.`,
      ``,
      `Transcript:`,
      lines.join("\n"),
    ].join("\n");
  ...
  async #deliverToOrigin(
    ...
    switch (platform) {
      ...
      case "evolution": {
        // Explicit Evolution target — used by group digests, which must never
        // go through the Meta Cloud API even when it is configured
        if (this.env.EVOLUTION_API_URL && this.env.EVOLUTION_API_KEY) {
          const { sendEvolutionMessage } = await import("./gateway/evolution.js");
          await sendEvolutionMessage(
            this.env.EVOLUTION_API_URL, this.env.EVOLUTION_API_KEY,
            `clop-${this.#userId}`, chatId, text,
  ...
  async executeReminder(payload: { eventId: string }): Promise<void> {
    ...
    const timeStr = date.toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short", timeZone: "UTC" });
    let msg = `Reminder: ${evt.title} (${timeStr} UTC)`;
    if (evt.location) msg += `\n${evt.location}`;
  ...
  #initSchema(): void {
    ...
    // WhatsApp group messages collected in digest mode, consumed by #runGroupDigest
    this.sql`CREATE TABLE IF NOT EXISTS group_messages (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    this.sql`CREATE INDEX IF NOT EXISTS idx_group_messages_chat ON group_messages (chat_jid, id)`;
